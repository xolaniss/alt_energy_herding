...
financial products \citep{nguyen2025}. Although the evidence from the
existing literature is mixed, returns from green financial products are
somewhat comparable to traditional financial products
\citetext{\citealp[see amongst
others,][]{decclesia2024}; \citealp{pastor2022}; \citealp[and][]{naqvi2022}}.
...
regressions \citep[ ]{koenker1978} in order to capture how herding
differs across various quantiles of the returns dispersion and in up and
down markets. Lastly, to establish a link between climate risk and
herding behaviour, we differentiating between transitional and physical
risks \citep{bua2024} and provide valuable insights on their potential
...
We adopt the popular definition herding behaviour, which follows from
the broader behaviour economics literature. In recent years behavioural
economics showed that human nature is prone to, amongst others,
emotional bias and heuristics
\citep{aggarwal2014animal, kahneman2003maps}. These biases can lead to
inefficient decision making, which can at times result in market
failures \citep{devenow1996}. In this context, herding behaviour is
imitative behivour or correlated trades by financial market agents that
can result in market volatility and crises when the market is not well
informed. This is why we are interested in herding behaviour in the
clean energy market.
...
herding behaviour tests. Next we estimate Equation~\ref{eq-herding} as
follows:
...
physical risk index\footnote{These indexes were constructed using the
  text-based approach of \citet{engle2020hedging}. This approach
  compares authorative text on climate change with European news sourced
  from Reuters - assuming that climate change events are covered in the
  news. The final sample had 1 million news articles from January 2005
  to October 2021. \citet{Bua2024} aggregate the articles to a daily
  frequency, thereby creating daily measures of \(TRI\) and \(PRI\)}.
For full details on the construction of \(TRI\) and \(PRI\), the reader
can refer to the paper of \citet{bua2024}.
